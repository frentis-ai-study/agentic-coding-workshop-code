...
async def mcp_client():
    ...
    async with Client(transport=stdio_server.mcp) as client:
        yield client


def test_stdio_server_import():
    """STDIO 서버를 임포트할 수 있는지 테스트."""
    try:
        import stdio_server

        assert stdio_server.mcp is not None
        assert stdio_server.mcp.name == "stdio-transport-server"
    except ImportError as e:
        pytest.fail(f"Failed to import stdio_server: {e}")


async def test_stdio_server_has_tools(mcp_client):
    """STDIO 서버가 도구를 가지고 있는지 테스트."""
